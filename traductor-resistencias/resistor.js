function construirHTMLResistor() {
  return `
          <svg
            xmlns:dc="http://purl.org/dc/elements/1.1/"
            xmlns:cc="http://creativecommons.org/ns#"
            xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
            xmlns:svg="http://www.w3.org/2000/svg"
            xmlns="http://www.w3.org/2000/svg"
            xmlns:sodipodi="http://sodipodi.sourceforge.net/DTD/sodipodi-0.dtd"
            xmlns:inkscape="http://www.inkscape.org/namespaces/inkscape"
            width="2160"
            height="1080"
            viewBox="0 0 2160 1080"
            id="svg2"
            version="1.1"
            inkscape:version="0.48.5 r10040"
            sodipodi:docname="resistor.svg">
            <defs
              id="defs4" />
            <sodipodi:namedview
              id="base"
              pagecolor="#ffffff"
              bordercolor="#666666"
              borderopacity="1.0"
              inkscape:pageopacity="0.0"
              inkscape:pageshadow="2"
              inkscape:zoom="0.12374369"
              inkscape:cx="504.15002"
              inkscape:cy="205.80139"
              inkscape:document-units="in"
              inkscape:current-layer="layer1"
              showgrid="false"
              units="in"
              inkscape:window-width="1366"
              inkscape:window-height="721"
              inkscape:window-x="-4"
              inkscape:window-y="-4"
              inkscape:window-maximized="1" />
            <metadata
              id="metadata7">
              <rdf:RDF>
                <cc:Work
                  rdf:about="">
                  <dc:format>image/svg+xml</dc:format>
                  <dc:type
                    rdf:resource="http://purl.org/dc/dcmitype/StillImage" />
                  <dc:title></dc:title>
                </cc:Work>
              </rdf:RDF>
            </metadata>
            <g
              inkscape:label="Layer 1"
              inkscape:groupmode="layer"
              id="layer1"
              transform="translate(0,27.6403)">
              <g
                id="g3825">
                <rect
                  y="318.11612"
                  x="32.324883"
                  height="48.487324"
                  width="2080.9143"
                  id="rect3004"
                  style="fill:#666666;stroke:none" />
                <path
                  inkscape:connector-curvature="0"
                  id="rect3776"
                  transform="translate(0,-1107.6403)"
                  d="m 798.5625,1308.5625 c -21.47944,0 -37.32619,4.0094 -49.46875,10.5625 -0.0526,0.028 -0.10377,0.065 -0.15625,0.094 -18.04664,8.9915 -27.35769,22.7119 -33.90625,37.4374 -14.51122,27.7533 -22.11341,59.0945 -55.8125,69.0938 -1.31397,0.3899 -2.62206,0.7469 -3.9375,1.125 l 0,46.2188 c 1.31639,0.3788 2.62251,0.766 3.9375,1.1562 64.74738,19.2121 12.93753,117.1875 139.34375,117.1875 0.73556,0 1.52761,-0.033 2.3125,-0.062 1.42733,0.029 2.86724,0.062 4.34375,0.062 30.2139,0 103.1066,-25.625 131.1875,-25.625 l 185.24995,0 c 28.081,0 100.9736,25.625 131.1876,25.625 1.4764,0 2.9164,-0.034 4.3437,-0.062 0.7849,0.03 1.5769,0.062 2.3125,0.062 126.4062,0 74.5964,-97.9754 139.3438,-117.1875 1.3149,-0.3902 2.6211,-0.7774 3.9374,-1.1562 l 0,-46.2188 c -1.3154,-0.3781 -2.6235,-0.7351 -3.9374,-1.125 -33.6993,-9.9993 -41.3013,-41.3405 -55.8126,-69.0938 -6.5485,-14.7255 -15.8596,-28.4459 -33.9062,-37.4374 -0.053,-0.029 -0.1037,-0.065 -0.1562,-0.094 -12.1426,-6.5531 -27.9894,-10.5625 -49.4688,-10.5625 -0.7233,0 -1.4833,0.034 -2.25,0.062 -1.4474,-0.029 -2.9088,-0.062 -4.4062,-0.062 -30.214,0 -103.1066,25.625 -131.1876,25.625 l -185.24995,0 c -28.0809,0 -100.9736,-25.625 -131.1875,-25.625 -1.49753,0 -2.95885,0.034 -4.40625,0.062 -0.76674,-0.029 -1.52669,-0.062 -2.25,-0.062 z"
                  style="fill:#d9bb7a;fill-opacity:1;stroke:#565248" />
                <path
                  id="stripe_1"
                  transform="translate(0,-27.6403)"
                  d="M 798.5625 228.5625 C 794.83997 228.5625 791.30046 228.67303 787.90625 228.90625 L 787.90625 511.1875 C 791.31332 511.35148 794.85648 511.4375 798.5625 511.4375 C 799.29806 511.4375 800.09011 511.404 800.875 511.375 C 802.30233 511.404 803.74224 511.4375 805.21875 511.4375 C 817.35302 511.4375 836.39057 507.29016 856.625 502.34375 L 856.625 237.65625 C 836.39057 232.70984 817.35302 228.5625 805.21875 228.5625 C 803.72122 228.5625 802.2599 228.597 800.8125 228.625 C 800.04576 228.596 799.28581 228.5625 798.5625 228.5625 z "
                  style="fill:#fdca01;fill-opacity:1;stroke:#986601" />
                <path
                  id="stripe_2"
                  transform="translate(0,-27.6403)"
                  d="M 901.0625 248.5625 L 901.0625 491.4375 C 915.3171 488.17202 927.92103 485.8125 936.40625 485.8125 L 969.75 485.8125 L 969.75 254.1875 L 936.40625 254.1875 C 927.92103 254.1875 915.3171 251.82798 901.0625 248.5625 z "
                  style="fill:#9e0097;fill-opacity:1;stroke:#430086;stroke-width:0.99000000000000021;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1" />
                <path
                  id="stripe_3"
                  transform="translate(0,-27.6403)"
                  d="M 1014.1875 254.1875 L 1014.1875 485.8125 L 1082.875 485.8125 L 1082.875 254.1875 L 1014.1875 254.1875 z "
                  style="fill:#f44800;fill-opacity:1;stroke:#980101" />
                <path
                  id="stripe_4"
                  transform="translate(0,-27.6403)"
                  d="M 1252.8438 228.5625 C 1246.3558 228.5625 1237.9014 229.76927 1228.3438 231.625 L 1228.3438 508.375 C 1237.9014 510.23073 1246.3558 511.4375 1252.8438 511.4375 C 1254.3202 511.4375 1255.7602 511.403 1257.1875 511.375 C 1257.9724 511.405 1258.7644 511.4375 1259.5 511.4375 C 1274.3045 511.4375 1286.6423 510.09139 1297.0312 507.6875 L 1297.0312 234 C 1286.8423 230.56088 1274.5655 228.5625 1259.5 228.5625 C 1258.7767 228.5625 1258.0167 228.597 1257.25 228.625 C 1255.8026 228.596 1254.3411 228.5625 1252.8438 228.5625 z "
                  style="fill:#c68401;fill-opacity:1;stroke:none" />
              </g>
            </g>
          </svg>

`;
}

class Resistor {
  // value debe ser un array
  constructor(value, container) {
    this.valorColores = {
      0: "black",
      1: "brown",
      2: "red",
      3: "orange",
      4: "yellow",
      5: "green",
      6: "blue",
      7: "violet",
      8: "grey",
      9: "white",
    };

    this.container = container;
    this.element = document.createElement("div");
    this.element.id = "resistor";
    this.element.innerHTML = construirHTMLResistor();
    this.value = parseInt(value);
    this.stripe1 = this.element.querySelector("#stripe_1");
    this.stripe2 = this.element.querySelector("#stripe_2");
    this.stripe3 = this.element.querySelector("#stripe_3");
    this.stripe4 = this.element.querySelector("#stripe_4");

    this.stripes = [this.stripe1, this.stripe2, this.stripe3, this.stripe4];
    this.valuesArray = [0, 0, 0, 0];

    this.container.append(this.element);
  }

  setResistorValue(number) {
    this.value = number;
    let numberString = number.toString();
    if (numberString.length == 1) {
      this.valuesArray = [0, number, 0, 0];
    } else if (numberString.length >= 2) {
      let leftover = numberString.substring(2); // "cdef"
      if (parseFloat(leftover) > parseFloat(numberString) / 20) {
        this.valuesArray = [
          parseInt(numberString[0]),
          parseInt(numberString[1]),
          numberString.length - 2,
          1,
        ];
      } else {
        this.valuesArray = [
          parseInt(numberString[0]),
          parseInt(numberString[1]),
          numberString.length - 2,
          0,
        ];
      }
    } else {
      this.valuesArray = [0, 0, 0, 0];
    }
  }

  colorStripes() {
    for (let i = 0; i < this.stripes.length - 1; i++) {
      this.stripes[i].style.fill = this.valorColores[this.valuesArray[i]];
    }
    if (this.valuesArray[this.stripes.length - 1] == 0) {
      this.stripes[this.stripes.length - 1].style.fill = "#c68401";
    } else {
      this.stripes[this.stripes.length - 1].style.fill = "#C0C0C0 ";
    }
  }
}

class Circuito {
  constructor(currentResistorElement, currentResistor, circuitContainer) {
    this.currentResistor = currentResistor;
    this.circuitContainerElement = circuitContainer;
    this.resistorList = [];
    this.currentResistorElement = currentResistorElement;
  }

  agregarCurrentResistorACircuito() {
    this.resistorList.push(this.currentResistor);
    this.circuitContainerElement.append(this.currentResistor.element);
    this.currentResistor = new Resistor(0, this.currentResistorElement);
  }

  sumarEnSerie() {
    let sum = 0;
    this.resistorList.forEach((resistor) => {
      sum += parseInt(resistor.value);
    });
    return sum;
  }

  sumarEnParalelo() {
    let sum = 0;
    this.resistorList.forEach((resistor) => {
      sum += 1 / parseFloat(resistor.value);
    });
    return 1 / sum;
  }
}

// () => {} es una arrow function, donde
document.addEventListener("DOMContentLoaded", () => {
  const resistorValue = document.querySelector("#valor_resistencia");
  const currentResistor = document.querySelector("#current_resistence");
  const circuitContainer = document.querySelector(".resistences_container");
  const resistor = new Resistor(0, currentResistor);
  const circuit = new Circuito(currentResistor, resistor, circuitContainer);

  const botonAgregarACircuito = document.querySelector("#add_to_circuit");
  const textoParalelo = document.querySelector("#paralelo");
  const textoEnSerie = document.querySelector("#serie");

  resistorValue.addEventListener("input", () => {
    circuit.currentResistor.setResistorValue(resistorValue.value);
    circuit.currentResistor.colorStripes();
  });

  botonAgregarACircuito.addEventListener("click", () => {
    circuit.agregarCurrentResistorACircuito();
    textoEnSerie.textContent = `Suma total de las resistencias de tu circuito en serie: ${circuit.sumarEnSerie()}Ω`;
    textoParalelo.textContent = `Suma total de las resistencias de tu circuito en paralelo: ${circuit.sumarEnParalelo()}Ω`;
  });
});
